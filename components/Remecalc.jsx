          ...
          <div className="heading-astro-remed relative flex flex-col w-full gap-3 sm:gap-2">
            ...
            <div className="flex  items-center gap-2 justify-start w-full">
              <div className="relative sm:py-3 grid w-full  items-center justify-center grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-8 z-10">
                {remedbox.map((rembx) => (
                  <div key={rembx.id} className="flex flex-col gap-2">
                    <Link
                      href={{
                        pathname: rembx.link,
                        query: { slug: rembx.slug },
                      }}
                      className="remedies-card-top hover:scale-105 head-wrap rounded-xl   text-center flex flex-col items-center justify-center gap-2"
                    >
                      <Image
          ...
          <div className="heading-astro-remed relative flex flex-col w-full gap-2">
            ...
            <div className="flex  items-center gap-3 sm:gap-2 justify-start w-full">
              <div className="relative sm:py-3 grid w-full  items-center justify-center grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 z-10">
                {remdosha.map((remd) => (
                  <div key={remd.id} className="flex flex-col gap-2">
                    <Link
                      href={{
                        pathname: remd.link,
                        query: { slug: remd.slug },
                      }}
                      className="remedies-card-top hover:scale-105 head-wrap rounded-xl text-center flex flex-col items-center justify-center"
                    >
                      <Image
                        className="w-full h-auto"
                        src={remd.src}
